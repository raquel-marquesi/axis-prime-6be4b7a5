...
import { Calendar, LayoutDashboard, Clock, AlertTriangle } from "lucide-react";
      ...
      <div className="space-y-6">
        ...
        {(has("calendar") || has("bonus") || has("deadlines_by_member")) && (
          <div className="grid gap-6 md:grid-cols-2">
            {has("calendar") && (
              <InternalCalendarWidget key={`calendar-${calendarKey}`} onEventClick={handleEventClick} />
            )}
                        ...
                        <div className="flex gap-2">
                          {member.overdue > 0 && (
                            <Badge variant="destructive" className="text-xs">
                              {member.overdue} atrasado{member.overdue > 1 ? "s" : ""}
                            </Badge>
                          )}
                          {member.pending > 0 && (
                            <Badge variant="secondary" className="text-xs">
                              {member.pending} pendente{member.pending > 1 ? "s" : ""}
                            </Badge>
                          )}
                          {member.pending === 0 && member.overdue === 0 && (
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 rounded-lg bg-red-500/10">
                  <span className="text-sm font-medium text-red-700">Próximos 30 dias</span>
                  <Badge variant="destructive">{stats?.contractsExpiring30 || 0}</Badge>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-yellow-500/10">
                  <span className="text-sm font-medium text-yellow-700">Próximos 60 dias</span>
                  <Badge className="bg-yellow-500">{stats?.contractsExpiring60 || 0}</Badge>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-blue-500/10">
                  <span className="text-sm font-medium text-blue-700">Próximos 90 dias</span>
                  <Badge variant="secondary">{stats?.contractsExpiring90 || 0}</Badge>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold tracking-tight text-foreground">
              Olá, <span style={{ color: "hsl(28 52% 44%)" }}>{profile?.full_name?.split(" ")[0] || "Usuário"}</span>!
            </h1>
