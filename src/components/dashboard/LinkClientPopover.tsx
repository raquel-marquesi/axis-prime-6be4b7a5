  ...
  const { data: clients } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from('clients')
        .select('id, razao_social, nome_fantasia, nome')
        .eq('is_active', true)
        .order('razao_social');
      return data || [];
    },
  ...
  const linkMutation = useMutation({
    mutationFn: async (clientId: string) => {
      const { error } = await supabase
        .from('client_aliases' as any)
        .insert({ alias: aliasName.toUpperCase(), client_id: clientId } as any);
      if (error) throw error;
    },
    onSuccess: () => {
      setLinked(true);
      toast.success('Alias vinculado! Na próxima sincronização, esse cliente será reconhecido automaticamente.');
      queryClient.invalidateQueries({ queryKey: ['sync-history'] });
      setTimeout(() => setOpen(false), 1500);
    },
    onError: (err: any) => {
      if (err.message?.includes('duplicate key')) {
        toast.error('Esse nome já está vinculado a um cliente.');
      } else {
      ...
      <PopoverContent className="w-72 p-2" align="start">
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground">
            Vincular <strong>"{aliasName}"</strong> a um cliente existente:
          </p>
            ...
            <div className="space-y-0.5">
              {filtered.length === 0 ? (
                <p className="text-xs text-muted-foreground p-2 text-center">
                  {clients ? 'Nenhum cliente encontrado' : 'Carregando...'}
                </p>
              ) : filtered.map(c => {
                const display = c.nome_fantasia || c.razao_social || c.nome || '—';
                ...
                return (
                  ...
                  >
                    ...
                    {secondary && (
                      <span className="block text-[10px] text-muted-foreground truncate">{secondary}</span>
                    )}
                  </button>
                );
              })}
