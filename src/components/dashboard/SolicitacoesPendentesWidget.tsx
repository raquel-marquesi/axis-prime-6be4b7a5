...
export function SolicitacoesPendentesWidget() {
  ...
  const topSolicitacoes = [...solicitacoes]
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <div>
          ...
          <CardDescription>Tarefas aguardando atenção</CardDescription>
        </div>
        {pendingCount > 0 && (
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="space-y-3">
            {topSolicitacoes.map((solicitacao) => {
              const OrigemIcon = origemIcons[solicitacao.origem] || Pencil;
              const isOverdue = solicitacao.data_limite && 
                new Date(solicitacao.data_limite) < new Date();
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-medium truncate">{solicitacao.titulo}</p>
                      <Badge
                        className={cn('text-xs text-white shrink-0', prioridadeColors[solicitacao.prioridade])}
                      >
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      {solicitacao.client && (
                        <span className="text-xs text-muted-foreground truncate">
                          {(solicitacao.client as any).razao_social || (solicitacao.client as any).nome}
                        </span>
                      )}
                      {solicitacao.data_limite && (
                        <span className={cn('text-xs', isOverdue ? 'text-red-500' : 'text-muted-foreground')}>
                          • Prazo: {format(new Date(solicitacao.data_limite), 'dd/MM', { locale: ptBR })}
                        </span>
                      )}
                    </div>
                  </div>
            ...
            <Button variant="ghost" className="w-full mt-2" asChild>
              <Link to="/solicitacoes" className="flex items-center justify-center gap-2">
                Ver todos os prazos
