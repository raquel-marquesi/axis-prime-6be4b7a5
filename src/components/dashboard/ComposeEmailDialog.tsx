...
export function ComposeEmailDialog({ open, onOpenChange, userEmail, replyTo }: ComposeEmailDialogProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast({ title: 'E-mail enviado!', description: `Mensagem enviada para ${to}` });
      onOpenChange(false);
      setTo(''); setSubject(''); setBody('');
    } catch (err) {
      toast({ title: 'Erro ao enviar', description: err instanceof Error ? err.message : 'Falha ao enviar e-mail', variant: 'destructive' });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Send className="h-5 w-5" />Nova Mensagem</DialogTitle>
          <DialogDescription>Enviar e-mail como {userEmail}</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex justify-end gap-2">
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={sending}><X className="h-4 w-4 mr-1" />Cancelar</Button>
          <Button onClick={handleSend} disabled={sending}><Send className="h-4 w-4 mr-1" />{sending ? 'Enviando...' : 'Enviar'}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
