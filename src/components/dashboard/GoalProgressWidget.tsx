  ...
  return (
    <div className={`relative w-full ${h} overflow-hidden rounded-full bg-secondary`}>
      <div
        ...
        style={{
          width: `${Math.min(clampedWidth, 100)}%`,
          backgroundColor: color,
        }}
      />
      {percentage >= 100 && (
        <div
          className={`absolute top-0 left-0 ${h} rounded-full opacity-30 animate-pulse`}
...
function useGoalProgress() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<{ own: GoalData | null; team: GoalData[]; all: GoalData[] }> => {
      const result: { own: GoalData | null; team: GoalData[]; all: GoalData[] } = {
        own: null,
        team: [],
        all: [],
      };
      ...
      const { data: areaGoals } = await supabase
        .from('area_goals')
        .select('area, monthly_goal');
      ...
      const { data: goalUserRoles } = await supabase
        .from('user_roles')
        .select('user_id, role')
        .in('role', GOAL_ROLES);
      ...
      const { data: rpcData, error: rpcError } = await supabase.rpc('get_goal_progress_data', {
      ...
      for (const row of (rpcData as any[]) || []) {
        weightedByUser.set(row.user_id, Number(row.total_weighted));
      }
      ...
      function buildGoalData(uid: string): GoalData {
        ...
        return {
          ...
          area,
          totalWeighted: memberWeighted,
          monthlyGoal: memberGoal,
          percentage: memberGoal > 0 ? Math.round((memberWeighted / memberGoal) * 100) : 0,
        };
      }
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base">
          <Target className="h-5 w-5" style={{ color: getProgressColor(own.percentage) }} />
          Atingimento de Meta
        </CardTitle>
        <CardDescription>
          {AREA_LABELS[own.area] || own.area} — {format(new Date(), 'MMMM yyyy')}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="flex items-end justify-between">
          <div>
            <span className="text-3xl font-bold" style={{ color: getProgressColor(own.percentage) }}>
              {own.percentage}%
            </span>
            <p className="text-xs text-muted-foreground mt-1">{getProgressLabel(own.percentage)}</p>
          </div>
          <div className="text-right text-sm text-muted-foreground">
            <p>{own.totalWeighted.toFixed(1)} / {own.monthlyGoal}</p>
            <p className="text-xs">produção ponderada</p>
          </div>
        </div>
        ...
        {own && own.monthlyGoal > 0 && (
          <div className="p-3 rounded-lg bg-primary/5 border border-primary/10 space-y-2">
            <div className="flex items-center justify-between text-sm">
              <span className="font-medium">Você ({own.percentage}%)</span>
              <span className="text-muted-foreground">{own.totalWeighted.toFixed(1)} / {own.monthlyGoal}</span>
            </div>
              ...
              <div key={m.userId} className="flex items-center gap-3 p-2 rounded-lg bg-muted/50">
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-bold text-xs" style={{ color: getProgressColor(m.percentage) }}>
                      {m.percentage}%
                    </span>
                  </div>
...
export function GoalProgressWidgetManager() {
  ...
  const byArea = all.reduce<Record<string, GoalData[]>>((acc, d) => {
    (acc[d.area] = acc[d.area] || []).push(d);
    return acc;
  }, {});

  const areaAverages = Object.entries(byArea).map(([area, members]) => ({
    area,
    avgPercentage: Math.round(members.reduce((s, m) => s + m.percentage, 0) / members.length),
    members,
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          Atingimento de Meta — Visão Global
        </CardTitle>
        <CardDescription>{format(new Date(), 'MMMM yyyy')}</CardDescription>
      </CardHeader>
          ...
          <div className="space-y-3">
            ...
            {areaAverages.map(({ area, avgPercentage }) => (
              <div key={area} className="space-y-1">
                <div className="flex items-center justify-between text-sm">
                  <span className="font-medium">{AREA_LABELS[area] || area}</span>
                  <span className="font-bold" style={{ color: getProgressColor(avgPercentage) }}>
                    {avgPercentage}%
                  </span>
                </div>
