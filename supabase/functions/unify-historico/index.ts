...
// Activity name aliases from Axis → normalized local names
...
// Client name aliases from Axis → client search terms
...
async function createMissingProcesses(db: any, batchSize: number, startOffset: number) {
  // Get distinct process numbers from historico_axis that don't exist in processes
  const { data: missing, error } = await db.rpc("get_missing_axis_processes", {
    p_offset: startOffset,
    p_limit: batchSize,
  });

  // Fallback: do it with a raw query approach
  // First get distinct axis process numbers in pages
  ...
  if (axErr) {
    return jsonResponse({ error: axErr.message }, 500);
  }

  if (!axisRows || axisRows.length === 0) {
    return jsonResponse({ status: "done", phase: "create_processes", inserted: 0, next_offset: null });
  }

  // Get unique process numbers from this batch
  ...
  for (const row of axisRows) {
    ...
    if (num && toCreate.includes(num) && !processMap.has(num)) {
      processMap.set(num, {
        ...
        id_cliente: null as string | null, // will try to resolve
        area: "trabalhista" as const,
      });
    }
  }

  // Try to resolve client IDs
  ...
  // Resolve clients for each process using axis client name
  for (const row of axisRows) {
    const num = row.numero_processo?.replace(/\s+/g, "").trim();
    ...
    if (proc && !proc.id_cliente && row.cliente) {
      ...
      if (!clientId) {
        // Fuzzy: search in client lookup keys
        for (const [key, id] of clientLookup.entries()) {
  ...
  // Need a fallback client for processes with no match
  // Find or use first active client as dummy
  ...
  for (const proc of processMap.values()) {
    if (!proc.id_cliente) {
      if (!fallbackClientId) continue; // Can't create without client
    ...
    if (insErr) {
      ...
      for (const p of batch) {
        ...
        if (!singleErr) inserted++;
          ...
          console.error("Process insert err:", singleErr.message, p.numero_processo);
        }
      }
    } else {
...
async function importHistorico(db: any, batchSize: number, startOffset: number) {
  ...
  const stats = {
    total: 0, inserted: 0, skipped_no_user: 0, skipped_no_date: 0,
  ...
  while (hasMore) {
    if (Date.now() - startTime > TIMEOUT) {
      console.log(`Timeout at offset ${offset}`);
      break;
    }
    ...
    for (const row of rows as any[]) {
      ...
      const actNorm = norm(row.tipo_atividade)
        .replace(/ELABORARACAO/g, "ELABORACAO");
        ...
        if (!clientId) {
          ...
          if (aliasTarget) {
            ...
            for (const [key, id] of clientMap.entries()) {
              if (key.includes(aN) || aN.includes(key)) {
                clientId = id;
                break;
              }
            }
